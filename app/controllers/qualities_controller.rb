class QualitiesController < ApplicationController
  ...
  def index
    if params[:search].blank?  
      @qualities = Quality.order(:quality_name).paginate(:page => params[:page], :per_page => 7)
    else  
      print("---------------Quality---------------")
      # byebug
      @parameter = params[:search].downcase  
      @qualities = Quality.search(@parameter).order(:quality_name).paginate(:page => params[:page], :per_page => 7)
      end
  end
  ...
  private
    ...
    def quality_params
      params.require(:quality).permit(:quality_name, :shade_no)
    end
end
